...
#if(uddhav==0)
pdftopdf_rotation_e
operator_plus(pdftopdf_rotation_e lhs,
...
pdftopdf_rotation_e
operator_subtract(pdftopdf_rotation_e lhs,
	  pdftopdf_rotation_e rhs) // {{{
{
...
pdftopdf_rotation_e
operator_negate(pdftopdf_rotation_e rhs) // {{{
{
	    			...
	    			float pheight)
{
    ...
    switch (r) 
    {
        ...
        case ROT_90:
            {
                const float tmp0 = rect->bottom;
                rect->bottom = rect->left;
                rect->left = pheight - rect->top;
                rect->top = rect->right;
                rect->right = pheight - tmp0;
        ...
        case ROT_180:
            {
                const float tmp1 = rect->left;
                rect->left = pwidth - rect->right;
                rect->right = pwidth - tmp1;

                const float tmp2 = rect->top;
                rect->top = pheight - rect->bottom;
                rect->bottom = pheight - tmp2;
            }
            break;

        case ROT_270:
            {
                const float tmp3 = rect->top;
                rect->top = pwidth - rect->left;
                rect->left = rect->bottom;
                rect->bottom = pwidth - rect->right;
                rect->right = tmp3;
...
void _cfPDFToPDFPageRect_set(_cfPDFToPDFPageRect *rect, 
